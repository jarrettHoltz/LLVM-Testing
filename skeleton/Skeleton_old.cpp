      ...
      for (auto &B : F) {
        ...
        for (auto &I : B) {
          ...
          if (I.getOpcode() == Instruction::Store) {
            ...
            for(auto U : pointer->users()){  // U is of type User*
              if (auto j= dyn_cast<Instruction>(U)){
                j->dump();
                if (j->getOpcode() == Instruction::Load) {
                  llvm::LoadInst *inner = dyn_cast<llvm::LoadInst>(j);
//                      j->dump();

//                   Value *value2 = inner->getValueOperand();
                  Value *pointer2 = inner->getPointerOperand();
                  std::cout << "inner" << std::endl;
                  for(auto u : j->users()){  // U is of type User*
                    if (auto J= dyn_cast<Instruction>(u)){
                      J->dump();
                    }
                  }
